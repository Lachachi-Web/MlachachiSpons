...
bot.on('message', async (msg) => {
  ...
  if (matchBtn(text, '📊 Statistiques')) {
    const clientCampaigns = await dbClient.query(`SELECT campaign_id FROM clients WHERE telegram_id = $1`, [chatId]);
  ...
  if (matchBtn(text, '🧾 Versements')) {
    const res = await dbClient.query(`SELECT amount, deposit_date FROM deposits WHERE telegram_id=$1 ORDER BY deposit_date DESC`, [chatId]);
    if (res.rows.length === 0)
      return bot.sendMessage(chatId, "⚠️ Aucun versement enregistré.");
    const lines = res.rows.map(r => `💵 ${r.amount} ${DEFAULT_CURRENCY} - ${new Date(r.deposit_date).toLocaleDateString('fr-FR')}`);
...
`;
    bot.sendMessage(chatId, message, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: 'Ouvrir WhatsApp', url: whatsappLink }]
        ]
      }
    });
  }
